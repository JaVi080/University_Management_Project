...
public class AdminDashboard {
        ...
        private VBox dashboardmain() {
            ...
            vBox.getChildren().addAll(logoView, homeButton, userButton, toolsButton, editButton, appsButton, exitButton);
    ...
    private GridPane dashboardcentre() {
        ...
        updateFeeButton.setOnAction(actionEvent -> pane.setCenter(Update_Fee()));
        logoutButton.setOnAction(actionEvent -> app.switchLayout(new MainLayout(app).getPane()));
    ...
    private  VBox Add_User() {
        ...
        Label class_name=new Label(" Class(Section)");
        TextField class_txt=new TextField();
        Text sec_txt=new Text("Select for student/teacher");
        HBox class_box=new HBox(30,class_name,class_txt,sec_txt);
        ...
        vbox.getChildren().addAll(txt,space,HBox1,HBox2,HBox3,HBox4,HBox5,HBox6,hbox7,hbox8,class_box,hbox9);
        FontStyling(vbox);
...
//updating fee
BorderPane Update_Fee(){
        Label fee_label=new Label("Update Fee per semester");
        TextField fee_txt=new TextField();
        Label choice_courses=new Label(" Department Name");
        TextField dept_choice=new TextField();

        Button update_button=new Button("Update Fee");


    //connection with databse to add department names from the saved info in Course Table
    update_button.setOnAction(e-> {
                try {
                    ...
                    connection.setAutoCommit(false);
                    Statement statement = connection.createStatement();
                    ResultSet resultSet = statement.executeQuery("select *from fee_table");

                    String sql = "update fee_table set fee= ? where dept=?";
                    PreparedStatement update_info = connection.prepareStatement(sql);

                    while (resultSet.next()) {
                        if (dept_choice.getText().equalsIgnoreCase(resultSet.getString(1))) {
                            update_info.setInt(1, Integer.parseInt(fee_txt.getText().toString()));
                            update_info.setString(2, resultSet.getString(1));
                            System.out.println(resultSet.getString(1));

                            update_info.executeUpdate();
                        }
                    }
                    connection.commit();
                    resultSet.close();
                ...
                //Dialogue box to cnfirm that fee is updated
        Dialog dialog=new Dialog();
                dialog.setContentText("Fee has been updated");
                dialog.setOnCloseRequest(event-> dialog.close());
                dialog.showAndWait();
            });

        VBox vbox1=new VBox(40,choice_courses,dept_choice,fee_label,fee_txt,update_button);
